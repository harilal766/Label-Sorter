...
class AmazonLabel(BaseLabel):
    ...
    def get_page_summary(self) -> dict:
        ...
        try:
            ...
            order_id_match = self.extract_order_id()
            ship_date_match = re.findall(self.ship_date_pattern,self.page_text)
            
            if self.find_page_type() == "Invoice":
                ...
                for row in product_rows:
                    ...
                    page_summary = {"item_name" : prod_name, "qty" : prod_qty}

                    if page_summary["item_name"] != None:
                        self.items.append(page_summary)
        except Exception as e:
            print(e)
        else:
